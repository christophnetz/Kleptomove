...
for(i in 1:gc){
  ...
  #Weights
  mtrxwP1[,i] = table(cut(tanh(x$agents$ann[,1] * 200), seq(minwv, maxwv, stepsize), right=T))/length(x$agents$ann[,1])
  mtrxwP2[,i] = table(cut(tanh(x$agents$ann[,2] * 200), seq(minwv, maxwv, stepsize), right=T))/length(x$agents$ann[,2])
  mtrxwP3[,i] = table(cut(tanh(x$agents$ann[,3] * 200), seq(minwv, maxwv, stepsize), right=T))/length(x$agents$ann[,3])
  mtrxwP4[,i] = table(cut(tanh(x$agents$ann[,4] * 200), seq(minwv, maxwv, stepsize), right=T))/length(x$agents$ann[,4])
  
  
  mtrxwH1[,i] = table(cut(tanh(x$agents$ann[,5] * 200), seq(minwv, maxwv, stepsize), right=T))/length(x$agents$ann[,5])
  mtrxwH2[,i] = table(cut(tanh(x$agents$ann[,6] * 200), seq(minwv, maxwv, stepsize), right=T))/length(x$agents$ann[,6])
  mtrxwH3[,i] = table(cut(tanh(x$agents$ann[,7] * 200), seq(minwv, maxwv, stepsize), right=T))/length(x$agents$ann[,7])
  mtrxwH4[,i] = table(cut(tanh(x$agents$ann[,8] * 200), seq(minwv, maxwv, stepsize), right=T))/length(x$agents$ann[,8])
}
...
GRA1 <- ggplot(data = melt(t(mtrxwP1)), aes(x=Var1, y=Var2, fill=value)) + labs(x="", y=expression('W'[H]*' Herbivore')) +
  geom_tile() + scale_fill_gradient2(low = "white", high = "blue", mid = "red", 
                                     ...
                                     name="Frequency", guide = FALSE) + geom_hline(yintercept = 0)+ theme(axis.title=element_text(size=30), axis.text = element_text(size = 22))
