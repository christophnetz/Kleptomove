...
namespace glsl { 
  ...
  namespace {
    ...
    glm::vec2 asVec2(glm::detail::tvec2<short> svec2)
    {
      return glm::vec2(svec2) * (1.0f / std::numeric_limits<short>::max());
    }


    glm::detail::tvec2<short> asSvec2(glm::vec2 v)
    {
      return glm::detail::tvec2<short>(v * (float)(std::numeric_limits<short>::max()));
    }


    // Homogeneous transformation
    glm::vec3 transformPoint(const glm::mat4& M, const glm::vec3& v)  
    {
      auto s = 1.0f / (M[0][3] * v.x + M[1][3] * v.y + M[2][3] * v.z + M[3][3]);
    ...
    {
      cmpeq_vertex_eps(const V4F_N3F_T2S& pivot) : pivot_(pivot) {}
    ...
    struct acObject
    {
      ...
      {
        glm::vec3 v0 = vert[triangles[face].vi[0]];
        glm::vec3 v1 = vert[triangles[face].vi[1]];
        glm::vec3 v2 = vert[triangles[face].vi[2]];
    ...
    private:
      ...
      static std::map<const std::string, apmf> actions_;
    };


    std::map<const std::string, acParser::apmf> acParser::actions_{
    ...
    {
      auto action = actions_.find(token);
      (action != actions_.end()) ? (this->*action->second)() : read_fail();
    }
    ...
    glm::vec3 acParser::vertexNormal(const acObject& obj, int face, int id)
    {
      ...
      cmpeq_normal_crease cmp_crease(normal, obj.crease);
      for (size_t i = 1; i < obj.triangles.size(); ++i)
      {
        const acTriangle& tri = obj.triangles[i];
        if ((id == tri.vi[0]) || (id == tri.vi[1]) || (id == tri.vi[2]))
        {
          glm::vec3 adjacent_normal = obj.face_normal(i);
