...
namespace glmutils {
  ...
  inline typename bbox::vec_type center(const bbox& b);

  template<typename bbox>
  inline typename bbox::value_type volume(const bbox& b);

  template<typename bbox>
  inline typename bbox::vec_type extent(const bbox& b);

  template<typename bbox>
  inline void inflate(bbox& b, const typename bbox::vec_type& dv);

  template<typename bbox>
  inline void include(bbox& lhs, const typename bbox::vec_type& rhs);

  template<typename bbox>
  inline void include(bbox& lhs, const bbox& rhs);

  template <typename bbox>
  inline typename bbox::vec_type clip(typename bbox::vec_type const&, bbox const& b);

  template <typename bbox>
  inline bbox clip(bbox const& a, bbox const& b);
  ...
  public:
    ...
    tbbox() {}                          //!< Default constructor. Result is undefined.
    explicit tbbox(const vec_type& v) { p_[0] = p_[1] = v; }  //!< Constructs a empty bounding box with center \a v.

    template<typename O>
    explicit tbbox(const tbbox<O>& box) { p_[0] = vec_type(box.p0()); p_[1] = vec_type(box.p1()); }

    //! Constructs a bounding box including a set of points.
    //! \param[in] n Number of points.
    //! \param[in] v Forward iterator referencing to \a vec_type
    template<typename IT>
    tbbox(const int n, IT v)
    {
      p_[0] = p_[1] = v[0];
    ...
    tbbox(const vec_type& v, value_type r) { p_[0] = (v-r);  p_[1] = (v+r); }

    //! Constructs a bounding box from two corners.
    tbbox(const vec_type& p0, const vec_type& p1)
    {
    ...
    //! Assignment
    tbbox<vec_type>& operator=(const tbbox<vec_type>& b)
    {
      p_[0] = b.p_[0];
      p_[1] = b.p_[1];
      return *this;
    ...
    const vec_type& operator[](int i) const { return p_[i]; }  //!< Access to corner \a i.
    vec_type& operator[](int i) { return p_[i]; }        //!< Access to corner \a i.

    vec_type& p0() { return p_[0]; }              //!< Returns min. corner.
    vec_type& p1() { return p_[1]; }              //!< Returns max. corner.
    const vec_type& p0() const { return p_[0]; }        //!< Returns min. corner.
    const vec_type& p1() const { return p_[1]; }        //!< Returns max. corner.
  };
  ...
  template<typename bbox>
  inline void move(bbox& b, const typename bbox::vec_type& dp)
  {
    b.p0() += dp;
    b.p1() += dp;
  }
  ...
  template <typename bbox>
  inline typename bbox::vec_type clip(typename bbox::vec_type const& pt, bbox const& b)
  {
